...
STRATEGY_CONFIGS = {
    ...
    'ims_reversal': {
        ...
        'fixed_params': {
            ...
            'pip_sizes': {sym: config.PIP_SIZE[sym] for sym in ['GBPNZD', 'AUDUSD', 'USA30', 'USDCHF', 'XAUUSD', 'AUDJPY', 'AUDCAD', 'USDCAD'] if sym in config.PIP_SIZE},
        },
        'param_grid': {},
    },
    'ims_reversal_8sym': {
    ...
    'ims_reversal_er': {
        # ER14>0.2 D1 filter — tests whether the expectancy boost holds OOS
        'class': ImsReversalStrategy,
        'timeframes': ['H4', 'M15', 'D1'],
        'symbols': ['GBPNZD', 'AUDUSD', 'USA30', 'USDCHF', 'XAUUSD', 'AUDJPY', 'AUDCAD', 'USDCAD'],
        'fixed_params': {
            'tf_htf': 'H4', 'tf_ltf': 'M15',
            'entry_mode': 'pending',
            ...
            'sl_buffer_pips': 0.0,
            'max_losses_per_bias': 1,
            'er_period': 14, 'er_threshold': 0.2, 'er_tf': 'D1',
            'pip_sizes': {sym: config.PIP_SIZE[sym] for sym in ['GBPNZD', 'AUDUSD', 'USA30', 'USDCHF', 'XAUUSD', 'AUDJPY', 'AUDCAD', 'USDCAD'] if sym in config.PIP_SIZE},
        },
        'param_grid': {},
    },
    ...
    'candle_confirmation_eurusd_vol_refine': {
        # Narrow regime follow-up around the best fixed EURUSD D1 candidate.
        # Question: does blocking high-volatility D1 regimes rescue the recent bad fold?
        'class': CandleConfirmationStrategy,
        'timeframes': ['D1', 'H1', 'M5'],
        'symbols': ['EURUSD'],
        'min_trades': 20,
        ...
        'param_grid': {
            'd1_vol_filter': ['off', 'block_high_pct'],
            'd1_vol_block_pct': [0.7, 0.8],
        },
    },
    'candle_confirmation_eurusd_direction_refine': {
        # Question: did one side drive the recent fold failure?
        'class': CandleConfirmationStrategy,
        'timeframes': ['D1', 'H1', 'M5'],
        'symbols': ['EURUSD'],
        'min_trades': 20,
    ...
    'candle_confirmation_eurusd_location_refine': {
        # Question: does avoiding D1-stretched closes improve robustness?
        'class': CandleConfirmationStrategy,
        'timeframes': ['D1', 'H1', 'M5'],
        'symbols': ['EURUSD'],
        'min_trades': 20,
        ...
        'param_grid': {
            'd1_location_filter': ['off', 'extreme_band'],
            'd1_long_max_pct': [1.0, 0.6],
            'd1_short_min_pct': [0.0, 0.4],
        },
    },
    'candle_confirmation_eurusd_session_refine': {
        # Question: does a simple session restriction help after regime filters?
        'class': CandleConfirmationStrategy,
        'timeframes': ['D1', 'H1', 'M5'],
        'symbols': ['EURUSD'],
        'min_trades': 20,
        ...
        'param_grid': {
            'blocked_hours': [
                (),
                (*range(20, 24), *range(0, 7)),   # allow 07-19 UTC
                (*range(20, 24), *range(0, 12)),  # allow 12-19 UTC
                (*range(17, 24), *range(0, 7)),   # allow 07-16 UTC
            ],
        },
    },
