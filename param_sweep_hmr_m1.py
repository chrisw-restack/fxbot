"""
Parameter sweep for HourlyMeanReversionStrategy (XAUUSD M1).
...
Grid:
  ...
  fractal_n          — bars each side for swing confirmation
                       (1=1min, 2=2min, 3=3min, 5=5min confirmation on M1)
  max_pullback_pips  — max intra-run pullback; 0 = off
  session            — asian / london / both

Run: python3 param_sweep_hmr_m1.py
"""
...
FRACTAL_N     = [1, 2, 3, 5]   # M1: 1=1min, 2=2min, 3=3min, 5=5min confirmation
...
# ── Display tables ────────────────────────────────────────────────────────────
def show_table(rows, title, sort_key, min_trades=50):
